...
export class AdminService {
  ...
  async getStats() {
    const [users, admins, tracks, courses, lessons] = await Promise.all([
    ...
    return { users, admins, tracks, courses, lessons };
  }
  ...
  async createTrack(dto: CreateTrackDto) {
    const clash = await this.prisma.track.findUnique({ where: { slug: dto.slug } });
    if (clash) throw new ConflictException('Já existe uma trilha com este slug');

    const maxIdx = await this.prisma.track.aggregate({ _max: { orderIndex: true } });
  ...
  async createCourse(dto: CreateCourseDto) {
    const track = await this.prisma.track.findFirst({
      where: { OR: [{ id: dto.trackId }, { slug: dto.trackId }] },
    });
    if (!track) throw new NotFoundException('Trilha não encontrada');

    const clash = await this.prisma.course.findUnique({
      where: { trackId_slug: { trackId: track.id, slug: dto.slug } },
    });
    if (clash) throw new ConflictException('Já existe um curso com este slug nesta trilha');
    ...
    if (isFree && autoEnrollOnAuth) await this.enrollAllUsersInCourse(course.id);
    return course;
  }
  ...
  async updateCourse(courseId: string, dto: UpdateCourseDto) {
    ...
    return this.prisma.course.update({
      ...
      data: {
        ...
        ...(dto.description !== undefined && { description: dto.description }),
        ...(dto.orderIndex !== undefined && { orderIndex: dto.orderIndex }),
        ...(dto.autoEnrollOnAuth !== undefined && { autoEnrollOnAuth: dto.autoEnrollOnAuth }),
      },
    });
  }
  ...
  async createModule(dto: CreateModuleDto) {
    ...
    if (clash) throw new ConflictException('Já existe um módulo com este slug neste curso');
    ...
    return this.prisma.module.create({
      data: { courseId: course.id, slug: dto.slug, title: dto.title, orderIndex },
    });
  }
  ...
  async createLesson(dto: CreateLessonDto) {
    ...
    if (clash) throw new ConflictException('Já existe uma aula com este slug neste módulo');
  ...
  async updateLesson(lessonId: string, dto: UpdateLessonDto) {
    ...
    if (dto.slug !== undefined && dto.slug !== existing.slug) {
      const clash = await this.prisma.lesson.findFirst({
        where: { moduleId: existing.moduleId, slug: dto.slug, NOT: { id: lessonId } },
      });
      if (clash) throw new ConflictException('Já existe uma aula com este slug neste módulo');
    }
