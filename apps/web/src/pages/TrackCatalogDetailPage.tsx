...
export function TrackCatalogDetailPage() {
  ...
  const enroll = useMutation({
    mutationFn: async () => {
      return apiFetch<{ enrolled: boolean; message: string }>(`/me/tracks/${trackId}/enroll`, {
        method: 'POST',
        token: token!,
      });
    },
  ...
  if (isError || !data) {
    return (
      <ErrorState
        title="Não foi possível carregar esta trilha."
  ...
  return (
    <div className="space-y-12">
      ...
      >
        ...
        <div className="absolute -bottom-16 left-1/3 h-48 w-48 rounded-full bg-indigo-400/10 blur-3xl" aria-hidden />

        <div className="relative border-b border-slate-900/5 bg-white/85 px-5 py-6 backdrop-blur-sm sm:px-8 sm:py-8">
          <div className="flex flex-col gap-8 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex gap-4 sm:gap-6">
              <div
                className={`relative flex h-[4.5rem] w-[4.5rem] shrink-0 items-center justify-center rounded-2xl border-2 border-slate-900/10 shadow-lg sm:h-[5.5rem] sm:w-[5.5rem] ${visual.iconBg} ${visual.iconColor}`}
              >
                <TrackIcon className="h-10 w-10 sm:h-12 sm:w-12" strokeWidth={2} aria-hidden />
                <span className="absolute -bottom-1 -right-1 flex h-7 w-7 items-center justify-center rounded-full border-2 border-white bg-amber-500 text-[10px] font-black text-white shadow-md">
            ...
            <div className="flex w-full flex-col gap-3 sm:max-w-sm lg:items-end">
