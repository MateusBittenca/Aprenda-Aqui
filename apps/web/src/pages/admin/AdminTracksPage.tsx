...
export function AdminTracksPage() {
  ...
  if (isLoading) return <PageLoader label="Carregando trilhas…" />;
  if (isError || !data) return <ErrorState title="Não foi possível carregar as trilhas." error={error ?? new Error()} />;

  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-white">Catálogo</h1>
        <p className="mt-1 text-sm text-slate-400">Gerencie trilhas de estudo, cursos, módulos e aulas.</p>
      </div>
      ...
      <section className="rounded-2xl border border-dashed border-amber-500/25 bg-amber-500/[0.03] p-5 sm:p-6">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h2 className="text-sm font-bold uppercase tracking-wide text-slate-400">Nova trilha</h2>
        </div>
        <CreateTrackForm />
      </section>
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-bold uppercase tracking-wide text-slate-500">
            Trilhas existentes <span className="text-slate-600">({data.length})</span>
          </h2>
        </div>
        {data.length === 0 ? (
              ...
              <li key={t.id}>
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {t.tagline && <p className="mt-0.5 truncate text-sm text-slate-400">{t.tagline}</p>}
                  </div>
...
function CreateTrackForm() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setTitle(''); setSlug(''); setTagline(''); setDescription('');
      queryClient.invalidateQueries({ queryKey: ['tracks'] });
    },
    onError: (e: unknown) => toast.error(e instanceof ApiError ? e.message : 'Erro ao criar trilha'),
  });

  return (
    <form className="grid gap-4 sm:grid-cols-2" onSubmit={(e) => { e.preventDefault(); mutation.mutate(); }}>
      <div className="sm:col-span-2">
        <label className={lbl}>Título *
          <input
            required
            value={title}
      ...
      <div>
        <label className={lbl}>Slug (URL) *
          <input required value={slug} onChange={(e) => setSlug(slugify(e.target.value))} placeholder="ex.: inteligencia-artificial" className={fieldMono} />
        </label>
      </div>
      <div>
      ...
      <div className="sm:col-span-2">
        <label className={lbl}>Descrição
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={2} className={field} />
        </label>
      </div>
      <div className="sm:col-span-2">
        <button type="submit" disabled={mutation.isPending} className="inline-flex items-center gap-2 rounded-xl bg-amber-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-amber-900/25 transition hover:bg-amber-500 disabled:opacity-50">
